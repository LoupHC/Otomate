...
boolean callback_openPath(SdFile& parentDir, char *filePathComponent,
    ...
    Expects the context object to be an instance of `SDEClass` and
  ...
  if (isLastComponent) {
    SDEClass *p_SD = static_cast<SDEClass*>(object);
...
boolean SDEClass::begin(uint8_t csPin) {
  /*
...
boolean SDEClass::begin(uint32_t clock, uint8_t csPin) {
  return card.init(SPI_HALF_SPEED, csPin) &&
...
SdFile SDEClass::getParentDir(const char *filepath, int *index) {
...
File SDEClass::open(const char *filepath, uint8_t mode) {
  /*

     Open the supplied file path for reading or writing.

     The file content can be accessed via the `file` property of
     the `SDEClass` object--this property is currently
...
/*
File SDEClass::open(char *filepath, uint8_t mode) {
  //
...
boolean SDEClass::exists(const char *filepath) {
  /*
...
//boolean SDEClass::exists(char *filepath, SdFile& parentDir) {
//  /*
//
...
boolean SDEClass::mkdir(const char *filepath) {
  /*
...
boolean SDEClass::rmdir(const char *filepath) {
  /*
...
boolean SDEClass::remove(const char *filepath) {
